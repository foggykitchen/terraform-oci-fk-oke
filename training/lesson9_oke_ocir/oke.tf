module "fk-oke" {
  ...
  api_endpoint_subnet_id        = module.fk_vcn.subnet_ids["api_endpoint"]

  is_lb_subnet_public = true # OKE LoadBalanacer will be public (Internet facing)
  lb_subnet_id        = module.fk_vcn.subnet_ids["lb"]

  is_nodepool_subnet_public = false # OKE NodePool will be private (no Internet facing)
  nodepool_subnet_id        = module.fk_vcn.subnet_ids["nodes"]
  pods_subnet_id            = module.fk_vcn.subnet_ids["nodes"]
}
